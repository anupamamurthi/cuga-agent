...
class TestAutoDiscoveryPattern:
    ...
    def test_cuga_agent_auto_loads_skills_on_init(self, tmp_path):
        """
        CugaAgent with no skills_dir still picks up .cuga/skills/ via cuga_folder.
        """
        ...
        skills_dir = tmp_path / ".cuga" / "skills"
        skills_dir.mkdir(parents=True)
        _skill(skills_dir, "brand.md", "# Brand\n\nAUTO_LOADED_SKILL_CONTENT")

        # Patch LLMManager so no real LLM credentials are needed
        with patch("cuga.sdk.LLMManager") as mock_llm_manager:
            mock_llm_manager.return_value.get_model.return_value = MagicMock()
            agent = CugaAgent(cuga_folder=str(tmp_path / ".cuga"), auto_load_policies=False)

        assert "AUTO_LOADED_SKILL_CONTENT" in agent._skills

    def test_cuga_agent_explicit_skills_dir_overrides_cuga_folder(self, tmp_path):
        """
        Explicit skills_dir takes precedence over .cuga/skills/ auto-discovery.
        """
        from unittest.mock import MagicMock, patch
        from cuga.sdk import CugaAgent

        # .cuga/skills/ has one skill
        auto_dir = tmp_path / ".cuga" / "skills"
        auto_dir.mkdir(parents=True)
        _skill(auto_dir, "auto.md", "# Auto Skill\n\nAUTO_CONTENT")

        # Explicit dir has a different skill
        explicit_dir = tmp_path / "custom_skills"
        explicit_dir.mkdir()
        _skill(explicit_dir, "explicit.md", "# Explicit Skill\n\nEXPLICIT_CONTENT")

        with patch("cuga.sdk.LLMManager") as mock_llm_manager:
            mock_llm_manager.return_value.get_model.return_value = MagicMock()
            agent = CugaAgent(
                cuga_folder=str(tmp_path / ".cuga"),
                skills_dir=str(explicit_dir),
                auto_load_policies=False,
            )

        assert "EXPLICIT_CONTENT" in agent._skills
        assert "AUTO_CONTENT" not in agent._skills
