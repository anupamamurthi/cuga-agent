"""
VideoQAAgent — LangGraph React agent version.

CUGA version:
    CugaAgent(model, tools=[transcribe_video, search_transcript, get_segment_at_time],
              plugins=[CugaSkillsPlugin(skills_dir)])

LangGraph version:
    create_react_agent(llm, tools, state_modifier=system_prompt, checkpointer=MemorySaver())

The three tools are identical — no changes to transcriber.py or index.py.
Skills are loaded manually from ./skills/video_qa.md.
Thread history uses LangGraph's MemorySaver instead of CugaCheckpointer.
...
# ---------------------------------------------------------------------------
# Tools — identical to the CUGA version
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# VideoQAAgent — same public API as the CUGA version
# ---------------------------------------------------------------------------

class VideoQAAgent:
    """
    High-level wrapper around a LangGraph React agent for video Q&A.

    Public API is identical to the CUGA version so run.py needs minimal changes:
        await agent.transcribe(video_path)
        answer = await agent.ask(question, thread_id=...)

    Internal change:
        CugaAgent → create_react_agent + MemorySaver
    """
    ...
    def _get_agent(self):
        if self._graph is None:
            ...
            log.info("VideoQAAgent LangGraph React agent ready")
        return self._graph

    # ------------------------------------------------------------------
    # Public API — identical interface to the CUGA version
    # ------------------------------------------------------------------
